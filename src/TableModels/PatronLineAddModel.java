 ...
 * @author Luiso
 */
public class PatronLineAddModel extends AbstractTableModel {
    ...
    public PatronLineAddModel() {
    }

    public PatronLineAddModel(ArrayList<Articulo> arl, ArrayList<PatronLine> pl) {
        ...
        for(int i=0; i<source.size(); i++){
            for(PatronLine patlin : pl)
                if(source.get(i).getArticle().getNumber() == patlin.getArticle().getNumber())
                    source.remove(i);
        }
    }
